...
import MultiLinkPage from './MultiLinkPage';
import MultiEdgeDrawer from './LoginDrawer';
const MultiHome = () => {
  ...
  const checkTagId = async () => {
    ...
    const response = await postData('multi/chkTagId', formData, true)
    ...
    <Grid sx={{ display: "flex", justifyContent: 'center' }}>
      <Grid container spacing={2} sx={{ display: display, width: 450 }}>
        ...
        <Grid item xs={12} class={tablet ? 'activate' : 'activate1'} sx={{ display: "flex", justifyContent: 'center', textAlign: "center", backgroundColor: '#F3B419', borderRadius: 2, paddingBottom: "16px", color: 'black', fontSize: { xs: 55, md: 34 } }}>
          Activate your Multi Link Card here !
        </Grid>
        <Grid item xs={12} sx={{ display: "flex", justifyContent: 'center' }} >
        <MultiEdgeDrawer tagId={id}/>
        </Grid>
      </Grid>
      <Grid container spacing={2} sx={{ display: displayPage, width: 430, mt: 0, background: "radial-gradient( #414141,#171717)",minHeight: '99.9vh' }}>
        <MultiLinkPage data={data} />
      </Grid>
    </Grid>
  )
}
